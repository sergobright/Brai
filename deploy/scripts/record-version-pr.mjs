...
import { explicitWorkFromPull, snapshotPull } from './accepted-preview-branches.mjs';

const payload = JSON.parse(process.env.BRAI_PR_JSON || '{}');
const pull = payload.pull_request ?? payload;
const repository = payload.repository?.full_name ?? process.env.GITHUB_REPOSITORY ?? 'sergobright/Brai';
