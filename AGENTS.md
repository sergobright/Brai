...
- Если во время задачи обнаружена процедурная проблема, ошибочный шаг, sandbox/escalation trap, нехватка инструмента/документации или вынужденный обходной путь, мешающий качественной работе, добавить запись в runtime Postgres `activities` с `activity_type_id = 'operation'`, `author = 'Codex'` и статусом `New`; `title` — короткое русское название задачи, `description_md` — что нужно сделать и какой результат должен получиться, `reason` — почему задача появилась: что случилось и какие были предпосылки. Системные значения, команды, пути, API names и аббревиатуры можно оставлять как есть; секреты не записывать. Пользовательские записи остаются `activity_type_id = 'action'`.
