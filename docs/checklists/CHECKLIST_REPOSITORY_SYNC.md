...
- [ ] For preview-class work, record release notes with `node scripts/brai-task.mjs release-notes --short "..." --details "..." --reason "..." --testing "..." --detail "<one change>::<description>"` and repeat `--detail` for every independent change; then run `scripts/brai-preview-handoff.sh`, let it wait through transient `queued` / `in_progress` states, and use its verified preview letter, URL, branch, and commit only after success.
