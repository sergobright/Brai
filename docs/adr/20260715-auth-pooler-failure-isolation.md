...
- Supavisor получает tenant `brightos-prod` для production и `brightos-nonprod` для Dev/Preview. Первый rollout сохраняет legacy tenant для безопасного отката.
