...
Brai использует self-hosted Temporal как обязательный контрольный журнал CI/CD для preview-веток и promotions. GitHub Actions по-прежнему запускает проверки и скрипты деплоя; Temporal ограничивает и записывает критические transitions. Долгие deployment Activities отправляют heartbeat, ждут завершения cancellation и владеют командой как POSIX process group, чтобы cancellation завершал всё локальное SSH/deploy-дерево до закрытия workflow.
...
- Минус: signal wrapper обязан сохранять worker до terminal Activity cancellation; быстрый выход wrapper не может оставлять deployment process group без владельца.
- Риск: outages Temporal блокируют strict delivery до восстановления.

## Проверка

Запускайте Temporal state tests, включая поведенческие signal/process-group tests, и запрашивайте workflow state при изменениях delivery или сбоях.
