...
| `deploy/scripts/complete-operation-activities.sh <operation:agent-task:id>` | `require_escalated` | Helper re-enters host deploy context and may write the runtime DB; local test-owned SQLite paths are sandbox-safe only for legacy tests. |
