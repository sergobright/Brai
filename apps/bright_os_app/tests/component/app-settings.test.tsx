...
describe("BrightOsApp settings", () => {
  ...
  it("shows when an Android OTA update is ready for restart", async () => {
    ...
    otaPlugin.getState.mockResolvedValue({
      activeBundleVersion: "0.0.7.1",
      nativeVersionName: "0.0.7.1",
      nativeBuild: "1",
      nativeVersionCode: 1,
      candidateBundleVersion: "0.0.8.1",
    ...
    expect(screen.getByText("0.0.8.1")).toBeInTheDocument();
    expect(screen.getByText("0.0.7.1 (1)")).toBeInTheDocument();
  });
  ...
  it.each([
    ...
    [
      "/data/user/0/world.brightos.bright_os_client/cache/bright-ota-downloads/0.0.8.1.zip: open failed: ENOENT (No such file or directory)",
