...
describe("Android OTA readiness bridge", () => {
  ...
  it("marks the active Android bundle ready once", async () => {
    ...
    plugin.getState.mockResolvedValue({ activeBundleVersion: "0.0.5.1" });
    plugin.markReady.mockResolvedValue({ activeBundleVersion: "0.0.5.1" });
    const { notifyAndroidOtaReady } = await import("@/shared/platform/ota");
    ...
    expect(plugin.markReady).toHaveBeenCalledWith({ bundleVersion: "0.0.5.1" });
  });
