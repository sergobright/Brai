...
public class BrightTimerNotificationService extends Service {
    ...
    private void requestServerStop() {
        ...
        networkExecutor.execute(() -> {
            BrightTimerServerState state = requestTimerState("POST", "/v1/timer/stop");
    ...
    private void syncFromServer() {
        networkExecutor.execute(() -> {
            BrightTimerServerState state = requestTimerState("GET", "/v1/timer/state");
            if (!destroyed) handler.post(() -> applyServerState(state));
        });
    }

    private void applyServerState(@Nullable BrightTimerServerState state) {
        if (destroyed) return;
        if (state == null) {
            scheduleSync();
            return;
    ...
    @Nullable
    private BrightTimerServerState requestTimerState(String method, String path) {
        HttpURLConnection connection = null;
        try {
            URL url = new URL(BuildConfig.BRIGHT_OS_ANDROID_API.replaceAll("/$", "") + path);
            ...
            String cookie = CookieManager.getInstance().getCookie(BuildConfig.BRIGHT_OS_ANDROID_API);
            ...
            return body.trim().isEmpty() ? null : BrightTimerServerState.fromJson(body);
    ...
    private Notification buildNotification(String startedAtUtc) {
        ...
        Intent stopIntent = new Intent(this, BrightTimerNotificationService.class);
        stopIntent.setAction(ACTION_REQUEST_STOP);

        Intent dismissedIntent = new Intent(this, BrightTimerNotificationService.class);
...
final class BrightTimerServerState {
    ...
    private BrightTimerServerState(@Nullable String activeStartedAtUtc) {
        this.activeStartedAtUtc = activeStartedAtUtc;
    }

    static BrightTimerServerState fromJson(String json) {
        ...
        return new BrightTimerServerState(matcher.find() ? clean(matcher.group(1)) : null);
    }
