...
Node runner every five minutes; Supabase Postgres decides which registered agent is due.
...
- Keep scheduled runtime agent state in Supabase Postgres without registering obsolete
...
is one systemd timer as the wakeup mechanism and one Postgres table for agent
due/lock/run state.

## Delivery Guard

This is a runtime/product change because it adds server runtime code, Postgres
